...
int main(int argc, char **argv) {
  ...
  auto tc = argv[1];
  if (tc[0] != '\0') {
    std::ifstream tc_file(tc, std::ios::binary | std::ios::ate);
    if (tc_file.is_open()) {
      std::streamsize size = tc_file.tellg();
      auto buffer = std::make_unique<uint8_t[]>(size);
      tc_file.seekg(0, std::ios::beg);
      if (tc_file.read(reinterpret_cast<char *>(buffer.get()), size)) {
        LLVMFuzzerTestOneInput(buffer.get(), size);
      }
    }
  }
  return EXIT_FAILURE;
